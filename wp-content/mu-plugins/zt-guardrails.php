...
/* -------------------------------------------------------------------------
 * Метки отдельно от рубрик
 * ---------------------------------------------------------------------- */

/**
 * Отвязать право на метки от права на рубрики.
 *
 * В WordPress управление обеими таксономиями открывается одним правом:
 * `manage_post_tags`, `edit_post_tags` и `delete_post_tags` ядро в
 * map_meta_cap сводит к `manage_categories`. То есть по умолчанию редактор,
 * которому нужны метки, получает вместе с ними право менять структуру сайта —
 * скорее всего, неожиданно для владельца.
 *
 * Фильтр оставляет права на метки самостоятельными, и дальше они выдаются
 * отдельно от прав на рубрики.
 *
 * Асимметрия не случайна. Метка — часть содержания: её придумывает автор по
 * ходу письма, и требовать для нового бренда камня участия разработчика
 * значило бы нарушить разделение ролей. Рубрика — структура: она в меню, в
 * спеке и в репозитории, а созданная из админки исчезнет при пересоздании
 * сервера, и исчезнет молча.
 *
 * @param string[] $caps Права, к которым ядро свело запрошенное.
 * @param string   $cap  Запрошенное право.
 * @return string[] Права для проверки.
 */
function zt_map_tag_caps( $caps, $cap ) {
	switch ( $cap ) {
		case 'manage_post_tags':
		case 'edit_post_tags':
		case 'delete_post_tags':
			return array( 'manage_post_tags' );
	}

	return $caps;
}
add_filter( 'map_meta_cap', 'zt_map_tag_caps', 10, 2 );

/**
 * Права, которых у редактора быть не должно.
 *
 * Роль `editor` большинство из них и не даёт — список нужен, чтобы
 * расхождение исправлялось само, если права окажутся выданы плагином или
 * вручную. Исключение — `manage_categories`: его роль даёт, и он здесь
 * именно затем, чтобы быть отобранным.
 *
...
function zt_editor_forbidden_caps() {
	return array(
		...
		'update_themes',
		'manage_options',
		// Рубрики — структура сайта, она задана в репозитории.
		'manage_categories',
	);
}
 ...
 *
 * Роль `editor` даёт большинство по умолчанию; список сторожит от случайного
 * изъятия. Два права роль не даёт и они добавляются:
 *
 * `manage_post_tags` — метки в обход рубрик, см. zt_map_tag_caps();
 *
 * `edit_theme_options` — глобальные стили и меню. Оформление разделено по
 * слоям: цвета, шрифты и меню редактор меняет сам, шаблоны остаются файлами
 * в репозитории. Известное следствие: этим же правом ядро открывает правку
 * шаблонов, отдельного права на стили в WordPress нет. Запрет на правку
 * шаблонов через админку остаётся правилом, а не механизмом, и относится
 * в том числе к разработчику — см. docs/theme-layers.md.
 *
 * @return string[] Список прав.
 */
...
function zt_enforce_roles() {
	...
	/*
	 * Право на метки теперь самостоятельное, а роль администратора о нём не
	 * знает: до фильтра выше оно сводилось к manage_categories, который у неё
	 * есть. Без этой строки разделение прав отобрало бы метки у разработчика.
	 */
	$admin = get_role( 'administrator' );
	if ( null !== $admin && ! $admin->has_cap( 'manage_post_tags' ) ) {
		$admin->add_cap( 'manage_post_tags' );
	}
