 ...
 * Основной расход — сам источник: превью WordPress создаёт уменьшенные, и
 * крупнейшее из них (1024 пикселя по длинной стороне) занимает единицы
 * мегабайт, а не столько же, сколько оригинал.
 *
 * Коэффициент 1.15 и запас 24 МБ получены замером на сервере 6 августа 2026,
 * а не из общих соображений. Замерялось загрузкой через сайт при
 * memory_limit 384 МБ:
 *
 *   12, 24, 48, 64, 80 Мпикс — превью создаются;
 *   90 и 100 Мпикс           — PHP останавливается по нехватке памяти
 *                              в class-wp-image-editor-gd.php на распаковке.
 *
 * Формула воспроизводит все семь исходов. Прежний коэффициент 2.2 предполагал
 * результат, сопоставимый с источником, и отклонял бы файлы вплоть до 48 Мпикс,
 * которые на деле обрабатываются за секунды: редактор получал бы отказ там,
 * где отказа нет.
 *
 * Замер привязан к memory_limit, а не к объёму памяти машины, поэтому при
 * изменении ZT_PHP_MEMORY_LIMIT формула остаётся верной. При смене версии
 * WordPress или набора размеров превью её стоит перепроверить.
 *
...
function zt_reject_oversized_image( $file ) {
	...
	$estimate = (int) ( $pixels * 4 * 1.15 ) + 24 * 1024 * 1024;
...
/*
 * Тот же заслон на втором пути загрузки. Медиабиблиотека и редактор блоков шлют
 * файл как $_FILES, и его разбирает wp_handle_upload. Но REST-точка /wp/v2/media
 * принимает и файл сырым телом запроса — тогда WordPress сохраняет его во
 * временный файл и передаёт в wp_handle_sideload, у которого свой фильтр.
 *
 * Без этой строки один и тот же файл на одной и той же точке входа вёл себя
 * по-разному в зависимости от того, как его прислал клиент: через $_FILES —
 * понятный отказ, сырым телом — фатальная ошибка PHP и код 500. Проверено на
 * сервере 6 августа 2026.
 */
add_filter( 'wp_handle_sideload_prefilter', 'zt_reject_oversized_image' );
